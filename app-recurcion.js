class Node {
  ...
  addNode(value) {
    ...
    if (existingChildNode) {
      existingChildNode.addNode(segments.slice(1).join("/")); // remove segment[0]
    } else {
      const node = new Node(segments[0], this);
      this.children.push(node);
  ...
  removeNode(value) {
    ...
    if (segments.length === 1) {
      ...
      this.children.splice(existingNodeIndex, 1); // remove segments[0]
    }
    ...
    if (segments.length > 1) {
      ...
      if (!existingChildNode) {
        throw new Error("Could not path.");
      }
...
console.log(user.findDepthFirst("username"));
console.log(user.findBreadthFirst("username"));
